# Теория по языку ассемблера и архитектуре ЭВМ(NASM, IA-32)
...
### _Пример_ функции с высокой локальностью
```c
int sumvec (int v [N]) {
  int i, sum = 0;
  for (i = 0; i < N; i++)
...
### _Пример_ функции с высокой пространственной локальностью
```c
...
### _Пример_ функции с низкой пространственной локальностью
```c
