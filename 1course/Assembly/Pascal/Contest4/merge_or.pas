// Задача merge_or: Слияние списков (объединение множеств)
// Даны две последовательности L1 и L2 из N и M целых чисел. Необходимо сформировать новую последовательность L, включив в нее по одному разу элементы, которые входят хотя бы в одну из последовательностей L1 и L2.

// В данной задаче запрещается использовать массивы. Для хранения последовательности необходимо использовать односвязные списки. Для решения задачи опишите процедуру merge_or(L1,L2, L), которая формирует требуемый список L.
...
procedure merge_or(L1, L2: List; var L: List);
{ Слияние списков }
begin
    L := nil;
    while L1 <> nil do
    begin
        if not find(L, L1^.data) then
            add(L, L1^.data);
        L1 := L1^.next;
    end;
    while L2 <> nil do
    begin
        if not find(L, L2^.data) then
            add(L, L2^.data);
        L2 := L2^.next;
    end;
