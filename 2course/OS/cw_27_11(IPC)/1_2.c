...
void process2(const char *filename, int N, key_t SEMAPHORE_KEY) {
  ...
  for (int i = 1; i < N; i++) {
    ...
    fprintf(file, "Process 2: PID - Hello World!\n");
    fflush(file);

    // Use the semaphore for 2nd process
    semaphore_operation.sem_num = 0;
    // Increment the semaphore value by 1x
    semaphore_operation.sem_op = 1;
...
int main(int argc, char **argv) {
  ...
  process2(shared_file, N, SEMAPHORE_KEY);
