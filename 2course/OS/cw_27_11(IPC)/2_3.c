...
void process3(const char *filename, int N, int msgId) {
  message_t msg;
  ...
  // Send a message to restart the cycle or end the operation
  msg.mtype = 1;
...
int main(int argc, char **argv) {
  ...
  // Send a message to start the cycle
  message_t msg;
  msg.mtype = 1;
  if (msgsnd(msgId, &msg, sizeof(msg.mtext), 0) == -1) {
    perror("msgsnd");
