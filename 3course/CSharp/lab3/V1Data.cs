...
public delegate void FYArray(double x, ref Complex y1, ref Complex y2);
...
public class V1DataList : V1Data
{
    ...
    public V1DataList(string key, DateTime date, double[] x, FDI F) : base(key, date)
    {
        ...
        foreach (var xi in x.Where(xi => xSet.Add(xi)))
        {
            DataItem item = F(xi);
            DataItems.Add(item);
        }
...
public class V1DataArray : V1Data
{
    ...
    public V1DataArray(string key, DateTime date, double[] x, FYArray F) : base(key, date)
    {
...
public class V1MainCollection : List<V1Data>, IEnumerable<DataItem>
{
    ...
    public V1MainCollection()
    {
        ...
        // Add one V1DataList
        double[] xYArray = { 0.0, 0.25, 0.5, 0.75, 1.0 };
        V1DataList dataList = new("DataListKey", DateTime.Now, xYArray, DataItemFunctions.FDI_Method);
        Add(dataList);
    }
    ...
    public IEnumerable<double>? XCoordinatesInMultipleDatasets
    {
        get
        {
            ...
            List<double> xYArray = xGrouped
                .Where(g => g.DataCount >= 2)
                .Select(g => g.X)
                .OrderBy(x => x)
                .ToList();

            return xYArray.Count != 0 ? xYArray : null;
        }
    }
}
