// Задача merge_and: Слияние списков (пересечение множеств)
// Даны две последовательности L1 и L2 из N и M целых чисел. Необходимо сформировать новую последовательность L, включив в нее по одному разу элементы, которые входят одновременно в обе последовательности L1 и L2.

// В данной задаче запрещается использовать массивы. Для хранения последовательности необходимо использовать односвязные списки. Для решения задачи опишите процедуру merge_and(L1,L2, L), которая формирует требуемый список L.
...
procedure merge_and(L1, L2: List; var L: List);
{ Слияние списков }
begin
    ...
    while L1 <> nil do
    begin
        if find(L2, L1^.data) and (not find(L, L1^.data))  then
            add(L, L1^.data);
        L1 := L1^.next;
    end;
